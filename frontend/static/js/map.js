    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            window.map = new AMap.Map('map-container', {
                zoom: 10,  // 设置默认缩放级别为10
                center: [120.3119, 31.4912],  // 设置地图中心点为无锡市
                viewMode: '2D',
                zooms: [1, 12],  // 设置缩放范围为1-12级
                layers: [
                    new AMap.TileLayer({
                        zIndex: 1,
...
function setupMapLayers() {
    ...
    try {
        ...
        window.vectorLayer = new window.AMap.TileLayer({
            ...
            getTileUrl: function(x, y, z) {
                if (z > 12) z = 12;
                return `/static/js/MAP_zxy/${z}/${x}/${y}.png`;
            }
        });               
        ...
        <div id="import-gps-modal" class="modal" style="display: flex; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background-color: rgba(0,0,0,0.5); justify-content: center; align-items: center; z-index: 1000;">
            <div class="modal-content" style="background-color: white; padding: 20px; border-radius: 5px; width: 80%; max-width: 600px; position: relative;">
                ...
                <div id="manual-input-container">
                    ...
                    <code>经度,纬度,时间(可选),地点(可选)</code><br>
                    例如: <code>116.403963,39.915119,2023-03-22 14:30:00,天安门</code></p>
        ...
        reader.onload = function(e) {
            try {
                ...
                if (!hasLongitude || !hasLatitude) {
                    alert('Excel文件必须包含"longitude"和"latitude"（或"经度"和"纬度"）列');
                    return;
                }
                ...
                jsonData.forEach((row, index) => {
                    ...
                    if (isNaN(longitude) || isNaN(latitude) || 
                        ...
                        invalidRows.push({
                            row: index + 1,
                            reason: '无效的GPS坐标',
                            value: `经度: ${row.longitude || row.经度}, 纬度: ${row.latitude || row.纬度}`
                        });
                        return;
                    }
                ...
                if (invalidRows.length > 0) {
                    const errorMessage = `发现${invalidRows.length}行无效数据：\n` +
                        ...
                        '\n\n请检查数据格式是否正确。\n' +
                        '有效的字段名：longitude/latitude 或 经度/纬度\n' +
                        '坐标范围：经度(-180到180)，纬度(-90到90)';
        ...
        function processGpsData() {
            ...
            if (isExcelActive) {
                ...
                for (let i = 0; i < lines.length; i++) {
                    ...
                    if (parts.length < 2) {
                        throw new Error(`第${i+1}行数据格式不正确，至少需要经度和纬度`);
                    }
                        ...
                        content: `
                            <div style="padding: 15px; min-width: 200px;">
                                ...
                                <div style="border-top: 1px solid #eee; padding-top: 10px;">
                                    ...
                                    <p style="margin: 5px 0;"><strong>GPS：</strong>${point.lng.toFixed(6)}, ${point.lat.toFixed(6)}</p>
                                </div>
                            </div>
                        `,
...
function updatePreview(data) {
    ...
    try {
        ...
        // 验证必要字段
        const requiredFields = ['经度', 'longitude', 'lng', '纬度', 'latitude', 'lat'];
        ...
        if (!hasRequiredFields) {
            previewContainer.insertAdjacentHTML('afterbegin', `
                <div style="margin-bottom: 10px; padding: 8px; background-color: #fff3cd; border: 1px solid #ffeeba; border-radius: 4px; color: #856404;">
                    <strong>警告：</strong> 未检测到必要的GPS字段（经度/longitude/lng 和 纬度/latitude/lat）
                </div>
            `);
        }
    } catch (error) {
